...
import com.gig8.motacoinj.core.VerificationException;
...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.motacoinSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testMotaCoinSerialization() throws Exception {
        ...
        // of transactions.
        Block block = new Block(params, blockBytes);
        assertArrayEquals(blockBytes, block.motacoinSerialize());
    }
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing motacoin serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.motacoinSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.motacoinSerialize().length && tx.length == 8);
        ...
        int origTxLength = 8 + 2 + 8 + 1 + 10 + 40 + 1 + 1 + 4; //motaoin: 4 for timestamp
        assertEquals(tx.motacoinSerialize().length, tx.length);
        ...
        assertEquals(block.length, block.motacoinSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
