...
import com.gig8.motacoinj.core.Wallet.SendRequest;
import com.gig8.motacoinj.crypto.*;
import com.gig8.motacoinj.params.MainNetParams;
import com.gig8.motacoinj.script.Script;
import com.gig8.motacoinj.script.ScriptBuilder;
import com.gig8.motacoinj.script.ScriptOpCodes;
import com.gig8.motacoinj.signers.StatelessTransactionSigner;
import com.gig8.motacoinj.signers.TransactionSigner;
import com.gig8.motacoinj.store.BlockStoreException;
import com.gig8.motacoinj.store.MemoryBlockStore;
import com.gig8.motacoinj.store.UnreadableWalletException;
import com.gig8.motacoinj.store.WalletProtobufSerializer;
import com.gig8.motacoinj.testing.*;
import com.gig8.motacoinj.utils.ExchangeRate;
import com.gig8.motacoinj.utils.Fiat;
import com.gig8.motacoinj.utils.Threading;
import com.gig8.motacoinj.wallet.*;
import com.gig8.motacoinj.wallet.WalletTransaction.Pool;
import com.google.common.collect.ImmutableList;
import com.google.common.collect.Lists;
import com.google.common.util.concurrent.ListenableFuture;
import com.google.protobuf.ByteString;
import com.gig8.motacoinj.wallet.Protos.Wallet.EncryptionType;
...
import static com.gig8.motacoinj.core.Utils.HEX;
import static com.gig8.motacoinj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    // For motacoin we shall send two cents, one cent for fee and another to meet minimum output requirement
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, boolean testEncryption) throws Exception {
        ...
        // will attach a small fee. Because the MotaCoin protocol makes it difficult to determine the fee of an
        ...
        try {
            ...
            assertEquals(valueOf(9, 1), e.missing); // motaoin: Miss nine coins and cent fee
        }
    ...
    public void balance() throws Exception {
        ...
        spend.params = MainNetParams.get(); // motaoin: Use normal selectable logic
        wallet.commitTx(spend);
        assertEquals(1, wallet.getPoolSize(WalletTransaction.Pool.PENDING));

        // Available and estimated balances should not be the same. We don't check the exact available balance here
        // because it depends on the coin selection algorithm.
        assertEquals(valueOf(4, 49), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // motaoin: minus fee
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), valueOf(0, 10));
        // Reserialize.
        Transaction send2 = new Transaction(params, send1.motacoinSerialize());
        assertEquals(nanos, send2.getValueSentFromMe(wallet));
        assertEquals(ZERO.subtract(valueOf(0, 11)), send2.getValue(wallet)); // motaoin: Cent fee
    }
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = new Transaction(params, tx.motacoinSerialize());
        try {
    ...
    public void bounce() throws Exception {
        ...
        inbound2.addOutput(new TransactionOutput(params, inbound2, coinHalf.subtract(CENT), myAddress)); // motaoin: Less a cent
        ...
        assertEquals(coin1.subtract(TWO_CENTS), wallet.getBalance()); // motaoin: Two cents loss in two transactions
    }
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        // motaoin: Make sure main net is used to make coins unselectable
        ...
        send2 = new Transaction(params, send2.motacoinSerialize());
        ...
        assertEquals(valueOf(1, 99), wallet.getBalance()); // motaoin: minus fee
        assertTrue(wallet.isConsistent());
    }
    ...
    public void doubleSpends() throws Exception {
        ...
        byte[] buf = send1.motacoinSerialize();
        buf[47] = 0;  // Break the signature: motacoinj won't check in SPV mode and this is easier than other mutations
        // motaoin: we used 47 thus adding 4 for the timestamp
        send1 = new Transaction(params, buf);
        wallet.commitTx(send2);
        wallet.allowSpendingUnconfirmedTransactions();
        assertEquals(value.subtract(CENT), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // motaoin: minus cent fee
        // Now spend the change. This transaction should die permanently when the mutant appears in the chain.
        Transaction send3 = checkNotNull(wallet.createSend(address, value.subtract(TWO_CENTS))); // motaoin: minus the first fee and then another required fee
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = new Transaction(params, t1.motacoinSerialize());
    ...
    public void spendToSameWallet() throws Exception {
        ...
        // motaoin: We will use MainNetParams so to make pending transaction unselectable
    	outbound1.params = MainNetParams.get();
        wallet.commitTx(outbound1);
        // We should have a zero available balance before the next block.
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(outbound1, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        // We should have a balance of 1 MOTA after the block is received.
        assertEquals(coin1.subtract(CENT), wallet.getBalance()); // motaoin: minus cent fee
    }
    ...
    public void pubkeyOnlyScripts() throws Exception {
        ...
        Transaction t2 = wallet.createSend(new ECKey().toAddress(params), value.subtract(CENT)); // motaoin: minus fee
        assertNotNull(t2);
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.motacoinSerialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.motacoinSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.motacoinSerialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.motacoinSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("motacoinj-unit-test", null);
    ...
    public void ageMattersDuringSelection() throws Exception {
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            txns[i] = sendMoneyToWallet(COIN.add(CENT), AbstractBlockChain.NewBlockType.BEST_CHAIN); // motaoin: Give room for fee
        }
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        // Spend it 
        SendRequest req = SendRequest.to(notMyAddr, CENT.multiply(11)); // motaoin: minus cent fee
        ...
        // motaoin: Check minimum fee is enforced
        ...
        // motaoin: Check less than cent change is disposed of.
        ...
        SendRequest request6 = SendRequest.to(notMyAddr, Coin.COIN.subtract(TWO_CENTS)); // motaoin: Extra cent for fee
        ...
        // We optimize for priority, so the output selected should be the largest one
        assertEquals(Coin.COIN.subtract(CENT), spend6.getOutput(0).getValue().add(spend6.getOutput(1).getValue())); // motaoin: minus cent fee

        // motaoin: All balance minus fee gives no change
        ...
        // motaoin: Used 1.00 coin, now 0.9 due to fee
        ...
        assertTrue(request15.tx.motacoinSerialize().length > 1000);
        request15.feePerKb = SATOSHI;
        wallet.completeTx(request15);
        assertEquals(CENT.add(SATOSHI), request15.tx.getFee()); // motaoin: Should add kb fee on top of required fee per extra kb
        ...
        assertTrue(request16.tx.motacoinSerialize().length > 1000);
        ...
        // motaoin: Check less than 1kb transaction
        ...
        assertEquals(outValue19, COIN.add(CENT)); // motaoin: We used two cents remember?
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        SendRequest request1 = SendRequest.to(notMyAddr, balance.subtract(CENT).subtract(SATOSHI)); // motaoin: minus fee
        wallet.completeTx(request1);
        assertEquals(SATOSHI.add(CENT), request1.tx.getFee()); // motaoin: cent fee added
    ...
    public void feePerKbCategoryJumpTest() throws Exception {
        ...
        // motaoin: With inputs of Coin plus cent we can pay 1.00 with no change
        ...
        // motaoin: We can pay 1.001 with coin and two cents but change is discarded
        ...
        // motaoin: We can pay 0.991 with coin and one cent but change is discarded so two cents are used instead to allow for change
    ...
    public void testCompleteTxWithExistingInputs() throws Exception {
        ...
        assertEquals(COIN.subtract(CENT.multiply(2)), request1.tx.getOutput(1).getValue()); // motaoin: cent fee
    ...
    public void testEmptyWallet() throws Exception {
        ...
        Transaction tx = createFakeTx(params, TWO_CENTS, myAddress); // motaoin: Need an extra cent for fee
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
    ...
    public void keyRotationRandom() throws Exception {
        ...
        assertEquals(CENT, tx.getOutput(0).getValue()); // motaoin: Two cents minus fee to one cent remaining.
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToMotaCoin();
    ...
    public void sendRequestMemo() throws Exception {
        receiveATransaction(wallet, myAddress);
        SendRequest sendRequest = SendRequest.to(myAddress, Coin.COIN.subtract(CENT)); // motaoin: minus fee
