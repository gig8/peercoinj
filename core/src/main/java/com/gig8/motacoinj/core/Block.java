...
import static com.gig8.motacoinj.core.Utils.doubleDigest;
import static com.gig8.motacoinj.core.Utils.doubleDigestTwoBuffers;

/**
 * <p>A block is a group of transactions, and is one of the fundamental data structures of the MotaCoin system.
 ...
 * <a href="http://www.motacoin.org/motacoin.pdf">the MotaCoin technical paper</a> for
...
public class Block extends Message {
    ...
    static final long ALLOWED_TIME_DRIFT = 60; // Same value as official client.

    /**
     * A constant shared by the entire network: how large in bytes a block is allowed to be. One day we may have to
     * upgrade everyone to change this, so MotaCoin can continue to grow. For now it exists as an anti-DoS measure to
    ...
    /** Constructs a block object from the MotaCoin wire format. */
    ...
    /**
     * Contruct a block object from the MotaCoin wire format.
    ...
    private void parseTransactions() throws ProtocolException {
        ...
        if (payload.length == cursor || payload.length - 1 == cursor) { // Include the -1 case for the null byte which is required for motacoin
    ...
    public int getOptimalEncodingMessageSize() {
        ...
        if (optimalEncodingMessageSize != 0)
            return optimalEncodingMessageSize;
        optimalEncodingMessageSize = motacoinSerialize().length;
        return optimalEncodingMessageSize;
    }
    ...
    /** Returns the version of the block data structure as defined by the MotaCoin protocol. */
     ...
     * {@link com.gig8.motacoinj.core.Block#getDifficultyTargetAsInteger()}. Note that this is <b>not</b> the same as
     * the difficulty value reported by the MotaCoin "getdifficulty" RPC that you may see on various block explorers.
    ...
    void addCoinbaseTransaction(byte[] pubKeyTo, Coin value) {
        ...
        coinbase.length = coinbase.motacoinSerialize().length;
