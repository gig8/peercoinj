...
import com.gig8.motacoinj.wallet.DefaultRiskAnalysis;
...
public class TransactionInput extends ChildMessage implements Serializable {
    ...
    protected void motacoinSerializeToStream(OutputStream stream) throws IOException {
        outpoint.motacoinSerialize(stream);
     ...
     * "from address" is not well defined in MotaCoin and you should not assume that senders of a transaction can
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the MotaCoin wiki for
     ...
     * @see TransactionOutPoint#getConnectedRedeemData(com.gig8.motacoinj.wallet.KeyBag)
     */
    @Nullable
    ...
    public TransactionInput duplicateDetached() {
        return new TransactionInput(params, null, motacoinSerialize(), 0);
    }
