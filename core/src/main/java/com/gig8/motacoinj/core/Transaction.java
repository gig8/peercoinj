...
import com.gig8.motacoinj.crypto.TransactionSignature;
...
import static com.gig8.motacoinj.core.Utils.*;
import static com.google.common.base.Preconditions.checkState;

/**
 * <p>A transaction represents the movement of coins from some addresses to some other addresses. It can also represent
 * the minting of new coins. A Transaction object corresponds to the equivalent in the MotaCoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of MotaCoin and have many powerful features. Read
 ...
 *
 * <p>All MotaCoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both motacoin and java serialization.
     ...
     * @param payload MotaCoin protocol formatted byte array containing message content.
    ...
    /**
     * Used by MotaCoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of MotaCoin share. In 2011 the value of a coinbase
    ...
    public void clearInputs() {
        ...
        this.length = this.motacoinSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on MotaCoin scripting and contracts
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, com.gig8.motacoinj.script.Script, ECKey, com.gig8.motacoinj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], com.gig8.motacoinj.core.Transaction.SigHash, boolean)}
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in MotaCoin 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // MotaCoin implementation. The reference client also contains a bug in handling this case:
    ...
    @Override
    protected void motacoinSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        uint32ToByteStreamLE(time, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.motacoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.motacoinSerialize(stream);
     ...
     * since MotaCoin 0.8+ a transaction that did not end its lock period (non final) is considered to be non
