...
public class FilteredBlock extends Message {
    ...
    @Override
    public void motacoinSerializeToStream(OutputStream stream) throws IOException {
        if (header.transactions == null)
            header.motacoinSerializeToStream(stream);
        else
            header.cloneAsHeader().motacoinSerializeToStream(stream);
        merkleTree.motacoinSerializeToStream(stream);
    }
