...
import com.gig8.motacoinj.net.MessageWriteTarget;
import com.gig8.motacoinj.net.StreamParser;
...
public abstract class PeerSocketHandler extends AbstractTimeoutHandler implements StreamParser {
    ...
    private MotaCoinSerializer.MotaCoinPacketHeader header;
    ...
    public PeerSocketHandler(NetworkParameters params, InetSocketAddress remoteIp) {
        serializer = new MotaCoinSerializer(checkNotNull(params));
    ...
    public int receiveBytes(ByteBuffer buff) {
        checkArgument(buff.position() == 0 &&
                buff.capacity() >= MotaCoinSerializer.MotaCoinPacketHeader.HEADER_LENGTH + 4);
        try {
     ...
     * called automatically by {@link com.gig8.motacoinj.net.NioClient} or
     * {@link com.gig8.motacoinj.net.NioClientManager} once the socket finishes initialization.
     */
    @Override
