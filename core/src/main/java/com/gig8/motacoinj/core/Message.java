...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the MotaCoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion MotaCoin protocol version.
    ...
    private void selfCheck(byte[] payload, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = motacoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom MotaCoin protocol.
    ...
    /**
     * Should only used by MotaCoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by MotaCoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeMotaCoinSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] motacoinSerialize() {
        byte[] payload = unsafeMotaCoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the motacoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeMotaCoinSerialize() {
        ...
        try {
            motacoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to motacoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the motacoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void motacoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw payload use motacoinSerialize().
     */
    void motacoinSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented motacoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so MotaCoinSerializer can avoid 2 instanceof checks + a casting.
     */
