...
import com.gig8.motacoinj.protocols.payments.Protos.PaymentDetails;
import com.gig8.motacoinj.core.TransactionConfidence.ConfidenceType;
import com.gig8.motacoinj.crypto.*;
import com.gig8.motacoinj.params.UnitTestParams;
import com.gig8.motacoinj.script.Script;
import com.gig8.motacoinj.script.ScriptBuilder;
import com.gig8.motacoinj.script.ScriptChunk;
import com.gig8.motacoinj.signers.LocalTransactionSigner;
import com.gig8.motacoinj.signers.MissingSigResolutionSigner;
...
import com.gig8.motacoinj.utils.BaseTaggableObject;
import com.gig8.motacoinj.utils.ExchangeRate;
import com.gig8.motacoinj.utils.ListenerRegistration;
 ...
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of motacoinj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.gig8.motacoinj.wallet.WalletFiles.Listener)}
...
public class Wallet extends BaseTaggableObject implements Serializable, BlockChainListener, PeerFilterProvider, KeyBag, TransactionBag {
     ...
     * {@link com.gig8.motacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS}. The returned key is stable until
    ...
    /**
     * An alias for calling {@link #currentKey(com.gig8.motacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link com.gig8.motacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #currentKey(com.gig8.motacoinj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #currentAddress(com.gig8.motacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link com.gig8.motacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * {@link com.gig8.motacoinj.wallet.DeterministicKeyChain}. When the parameter is
     * {@link com.gig8.motacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} the returned key is suitable for being put
    ...
    /**
     * An alias for calling {@link #freshKey(com.gig8.motacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link com.gig8.motacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #freshKey(com.gig8.motacoinj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #freshAddress(com.gig8.motacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link com.gig8.motacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * key, after loading a wallet produced by previous versions of motacoinj. If the wallet is encrypted the key
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.gig8.motacoinj.wallet.WalletFiles.Listener)}
    ...
    /** See {@link com.gig8.motacoinj.wallet.DeterministicKeyChain#setLookaheadSize(int)} for more info on this. */
    ...
    /** See {@link com.gig8.motacoinj.wallet.DeterministicKeyChain#setLookaheadThreshold(int)} for more info on this. */
     ...
     * See {@link com.gig8.motacoinj.wallet.DeterministicKeyChain#markKeyAsUsed(DeterministicKey)} for more info on this.
     */
     ...
     * @throws com.gig8.motacoinj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.gig8.motacoinj.crypto.KeyCrypter,
     ...
     * {@link com.gig8.motacoinj.crypto.KeyCrypterScrypt}.
     *
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (isDeadCoinbase) {
            ...
            // we could. MotaCoin nodes on the network have deleted the dependent transactions from their mempools
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), Utils.HEX.encode(tx.unsafeMotaCoinSerialize()));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), Utils.HEX.encode(tx.unsafeMotaCoinSerialize()));
                }
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s MOTA (available: %s MOTA) in:%n",
     ...
     * of {@link com.gig8.motacoinj.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
     ...
     * you can use {@link com.gig8.motacoinj.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    public static class SendResult {
        /** The MotaCoin transaction message that moves the money. */
    ...
    public enum MissingSigsMode {
        ...
        USE_DUMMY_SIG,
        /**
         * If signature is missing, {@link com.gig8.motacoinj.signers.TransactionSigner.MissingSignatureException}
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.gig8.motacoinj.core.Wallet#getBalance()} - the fees required
        ...
        /**
         * If not null, the {@link com.gig8.motacoinj.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
     ...
     *
     * @param address The MotaCoin address to send the money to.
     ...
     * @throws ExceededMaxTransactionSize if the resultant transaction is too big for MotaCoin to process (try breaking up the amounts of value)
     */
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            // Check size.
            int size = req.tx.motacoinSerialize().length;
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, Coin baseFee, Coin feePerKb) {
        ...
        int size = tx.motacoinSerialize().length;
        size += estimateBytesForSigning(coinSelection);
        
        // motaoin: Always add required fee
                ...
                for (int i = 0; i < tx.getOutputs().size(); i++) {
                    ...
                    try {
                        if (isTxOutputBloomFilterable(out)) {
                            ...
                            filter.insert(outPoint.motacoinSerialize());
                        }
                    } catch (ScriptException e) {
    ...
    public FeeCalculation calculateFee(SendRequest req, Coin value, List<TransactionInput> originalInputs,
        ...
        while (true) {
            ...
            // motaoin: We always include the required fee
            ...
            // If change is < 0.01 MOTA, we will need to discard the change
            ...
            if (change.signum() > 0) {
                ...
                size += changeOutput.motacoinSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
            ...
            size += req.tx.motacoinSerialize().length;
     ...
     * sending transactions to the MotaCoin network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable com.gig8.motacoinj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws com.gig8.motacoinj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
    ...
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign) {
        ...
        try {
            ...
            checkState(rekeyTx.motacoinSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            return rekeyTx;
     ...
     * {@link com.gig8.motacoinj.core.PeerFilterProvider} interface and generally should not be used directly by apps.
