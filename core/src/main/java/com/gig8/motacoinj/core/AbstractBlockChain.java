 ...
 * the lightweight SPV mode, a {@link com.gig8.motacoinj.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.gig8.motacoinj.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of MotaCoin
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    ...
                    if (clone)
                        tx = new Transaction(tx.params, tx.motacoinSerialize());
