...
package com.gig8.motacoinj.crypto;
...
/**
 * A TransactionSignature wraps an {@link com.gig8.motacoinj.core.ECKey.ECDSASignature} and adds methods for handling
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * and generally violates people's mental model of how MotaCoin should work, thus, non-canonical signatures are now
     ...
     * of the type used by MotaCoin we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeToMotaCoin() {
        try {
     ...
     */
    public static TransactionSignature decodeFromMotaCoin(byte[] bytes, boolean requireCanonical) throws VerificationException {
        // MotaCoin encoding is DER signature + sighash byte.
        ...
        // In MotaCoin, any value of the final byte is valid, but not necessarily canonical. See javadocs for
