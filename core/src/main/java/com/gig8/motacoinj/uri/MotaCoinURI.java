...
import com.gig8.motacoinj.core.AddressFormatException;
import com.gig8.motacoinj.core.Coin;
import com.gig8.motacoinj.core.Monetary;
...
/**
 * <p>Provides a standard implementation of a MotaCoin URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code motacoin:<address>}</li>
 * <li>{@code motacoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 */
public class MotaCoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(MotaCoinURI.class);
    ...
    public static final String PEERCOIN_SCHEME = "motaoin";
    ...
    /**
     * Constructs a new MotaCoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws MotaCoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public MotaCoinURI(String uri) throws MotaCoinURIParseException {
        this(MainNetParams.get(), uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid MotaCoinURI object.
     *
     ...
     *
     * @throws MotaCoinURIParseException If the input fails MotaCoin URI syntax and semantic checks.
     */
    public MotaCoinURI(NetworkParameters params, String input, String prefix) throws MotaCoinURIParseException {
        checkNotNull(input);
        ...
        try {
            ...
            throw new MotaCoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  motacoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form motacoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the motacoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : motacoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (input.startsWith(prefix + "://")) {
            ...
            throw new MotaCoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new MotaCoinURIParseException("No data found after the " + prefix + " prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a MotaCoin address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new MotaCoinURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new MotaCoinURIParseException("No address and no r= parameter found");
        }
    }

    /**
     * Constructs a new object by trying to parse the input as a valid MotaCoin URI.
     *
     ...
     * @throws MotaCoinURIParseException If the input fails MotaCoin URI syntax and semantic checks.
     */
    public MotaCoinURI(NetworkParameters params, String input) throws MotaCoinURIParseException {
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws MotaCoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new MotaCoinURIParseException("Malformed MotaCoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new MotaCoinURIParseException("Malformed MotaCoin URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws MotaCoinURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new MotaCoinURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The MotaCoin Address from the URI, if one was present. It's possible to have MotaCoin URI's with no address if a
     ...
     *         10,000,000 units is 1 MOTA. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("MotaCoinURI[");
        boolean first = true;
    ...
    public static String convertToMotaCoinURI(Address address, Coin amount, String label, String message) {
        return convertToMotaCoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple MotaCoin URI builder using known good fields.
     * 
     * @param address The MotaCoin address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the MotaCoin URI
     */
    public static String convertToMotaCoinURI(String address, @Nullable Coin amount, @Nullable String label,
