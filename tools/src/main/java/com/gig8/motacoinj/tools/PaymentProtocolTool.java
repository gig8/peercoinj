...
/** Takes a URL or motacoin URI and prints information about the payment request. */
public class PaymentProtocolTool {
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("Provide a motacoin URI or URL as the argument.");
            return;
        }
        dump(args[0]);
    }
    ...
    private static void dump(String arg) {
        try {
            ...
            } else if (uri.getScheme().equals("motacoin")) {
                MotaCoinURI bcuri = new MotaCoinURI(arg);
                final String paymentRequestUrl = bcuri.getPaymentRequestUrl();
                if (paymentRequestUrl == null) {
                    System.err.println("No r= param in motacoin URI");
                    return;
                }
                session = PaymentSession.createFromMotaCoinUri(bcuri).get();
            } else {
            ...
            StringBuilder output = new StringBuilder(
                    format("MotaCoin payment request, version %d%nDate: %s%n", version, session.getDate()));
